...
GEP_UNITTEST_TEST(math3d, mat4)
{
    ...
    // test mat4 multiplication with direction
    {
        ...
        GEP_ASSERT(epsilonCompare(result2, vec3(0.900165498f, 1.70065641f, 3.20896721f)), "multiplying a mat4 with a direction does not correctly rotate");
    }
    ...
    // test mat4 multiplication with position
    {
        ...
        GEP_ASSERT(epsilonCompare(result2, vec3(0.900165498f, 1.70065641f, 3.20896721f)), "multiplying a mat4 with a position does not correctly rotate");
    }

    // test sanity of rotationMatrixXYZ function
    {
        vec3 result = mat4::rotationMatrixXYZ(vec3(0, 0, 90)).transformDirection(vec3(1, 0, 0));
        GEP_ASSERT(epsilonCompare(result, vec3(0, 1, 0)), "rotation matrix rotates in the wrong direction");
    }
...
GEP_UNITTEST_TEST(math3d, aabb)
{
	...
	//test move
	{
		AABB box = AABB(vec3(-20, -20, -20), vec3(-10, -10, -10));
		AABB box2 = box.move(vec3(15, 15, 15));
		GEP_ASSERT(epsilonCompare(box2.getMin(), vec3(-5, -5, -5)));
		GEP_ASSERT(epsilonCompare(box2.getMax(), vec3(5, 5, 5)));
	}
}
...
GEP_UNITTEST_TEST(math3d, ray)
{
	Plane p1 = Plane(vec3(3, 0, 0), vec3(1, 0, 0));
	Ray r1 = Ray(vec3(0, 0, 0), vec3(1, 0, 0));

	float result = r1.intersectWith(p1);
	GEP_ASSERT(epsilonCompare(result, 3.0f));

	p1 = Plane(vec3(3, 0, 0), vec3(0, 1, 1));
	r1 = Ray(vec3(0, 1, 0), vec3(1, 0, 0));

	result = r1.intersectWith(p1);
	GEP_ASSERT(result != result); // this checks for NaN, std::isnan is not yet available in Visual Studio
...
{
    {
        ...
        GEP_ASSERT(epsilonCompare(noRot.x, 0.0f) && epsilonCompare(noRot.y, 0.0f) && epsilonCompare(noRot.z, 0.0f) && epsilonCompare(noRot.angle, 1.0f));
    }

    {
        Quaternion q(vec3(0.0f, 0.0f, 1.0f), 90.0f);
        vec3 v(1.0f,0.0f,0.0f);
        vec3 result = q.toMat3() * v;
        GEP_ASSERT(result.epsilonCompare(vec3(0.0f, 1.0f, 0.0f)), "Quaternion Rotation fail", result.x, result.y, result.z);

        result = q.toMat4().transformDirection(v);
        GEP_ASSERT(result.epsilonCompare(vec3(0.0f, 1.0f, 0.0f)), "Quaternion Rotation fail", result.x, result.y, result.z);

        q = Quaternion(mat4::rotationMatrixXYZ(vec3(0, 0, 90.0f)).rotationPart());
        result = q.toMat3() * v;
        GEP_ASSERT(result.epsilonCompare(vec3(0.0f, 1.0f, 0.0f)), "Quaternion Rotation fail", result.x, result.y, result.z);
    }

    {
        Quaternion q(vec3(1.0f, 0.0f, 0.0f), 90.0f);
        Quaternion r(vec3(0.0f, 0.0f, 1.0f), 90.0f);

        vec3 result = (q * r).toMat3() * vec3(0.0f, 1.0f, 0.0f);
        GEP_ASSERT(result.epsilonCompare(vec3(-1.0f, 0.0f, 0.0f)));
    }

    {
        mat4 m1 = mat4::rotationMatrixXYZ(vec3(13.0f, 17.0f, 31.0f));
        mat4 m2 = Quaternion(m1.rotationPart()).toMat4();
        for(size_t i=0; i< 16; i++)
        {
            GEP_ASSERT(epsilonCompare(m1.data[i], m2.data[i]));
        }
    }
