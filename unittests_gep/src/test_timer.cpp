...
GEP_UNITTEST_TEST(Timer, Timer)
{
    Timer timer;
    {
        ...
        GEP_ASSERT(ftime > 0.0004f && ftime < 0.0006f, "the time elapsed since the initialization of the timer should be roughly 0.0005 s", ftime);
        auto dtime = timer.getTimeAsDouble();
        GEP_ASSERT(dtime > 0.0004 && dtime < 0.0006, "the time elapsed since the initialization of the timer should be roughly 0.0005 s", dtime);
    }
