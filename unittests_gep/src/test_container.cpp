...
GEP_UNITTEST_TEST(Container, hashmap)
{
  ...
  {
      auto cstring_Key = "Hello";
      std::string stdstring_Key = cstring_Key;

      GEP_ASSERT(cstring_Key != stdstring_Key.c_str());

      Hashmap<const char*, int, StringHashPolicy, SimpleLeakCheckingAllocatorPolicy> map;
      auto& cstring_Value = map[cstring_Key];
      auto& stdstring_Value = map[stdstring_Key.c_str()];

      GEP_ASSERT(&cstring_Value == &stdstring_Value, "C-String keys are not compared correctly!");
  }
  ...
  //test string -> test struct
  {
      ...
      GEP_ASSERT(&map1[ConstantString("1")] != nullptr, "Hashmap operator[] is not implemented (properly)!");
  ...
  // test pseudo fill
  {
      Hashmap<Collision, int> map5;

      for(int i = 0; i<100; i++)
      {
          map5[Collision(i, i)] = i; // insert
          map5.remove(Collision(i, i)); // remove
          GEP_ASSERT(map5.exists(Collision(i, i)) == false); // lookup
      }
      GEP_ASSERT(!map5.isPseudoFull());
