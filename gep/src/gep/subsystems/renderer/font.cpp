...
// IFontLoader
gep::IFontLoader::IFontLoader(const char* resourceId) : 
...
gep::Font* gep::FontFileLoader::loadResource(Font* pInPlace)
{
    ...
    {
        char name[256];
		sprintf_s(name, "%.*s %d", m_resourceId.length()-4, m_resourceId.c_str(), m_fontSize);
        pInPlace = new Font(name, this);
    }
    pInPlace->unload();
    pInPlace->load(m_resourceId.c_str(), m_fontSize);
    return pInPlace;
}
...
gep::Texture2DFromFontLoader::Texture2DFromFontLoader(ResourcePtr<Font> pFont) :
	ITexture2DLoader(pFont->getLoader()->getResourceId()),
	m_pFont(pFont)
{
