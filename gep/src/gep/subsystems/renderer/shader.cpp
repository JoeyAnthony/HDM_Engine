...
gep::IShaderLoader::IShaderLoader(const char* resourceId) : 
...
gep::Shader* gep::ShaderFileLoader::loadResource(Shader* pInPlace)
{
    ...
    try {
        result->loadFromFX(m_resourceId.c_str());
        return result;
    }
...
void gep::ShaderFileLoader::postLoad(ResourcePtr<IResource> pResource)
{
    ...
    {
        m_isRegistered = true;
        g_globalManager.getResourceManager()->registerLoaderForReload(m_resourceId.c_str(), this, pResource);
    }
}
