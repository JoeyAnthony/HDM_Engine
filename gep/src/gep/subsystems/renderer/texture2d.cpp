...
gep::ITexture2DLoader::ITexture2DLoader(const char* resourceId) : 
...
gep::DummyTexture2DLoader::DummyTexture2DLoader() : 
	ITexture2DLoader("<Dummy Texture2D>")
{
}
...
gep::Texture2DFileLoader::~Texture2DFileLoader()
{
    if(m_isRegistered)
    {
        g_globalManager.getResourceManager()->deregisterLoaderForReload(m_resourceId.c_str(), this);
    }
}
...
gep::Texture2D* gep::Texture2DFileLoader::loadResource(Texture2D* pInPlace)
{
    ...
    {
        result = m_pRenderer->createTexture2D(m_resourceId.c_str(), this, TextureMode::Static);
        isInPlace = false;
    }
    try {
        DDSLoader loader(&g_stdAllocator);
        loader.loadFile(m_resourceId.c_str());
        if(loader.isCubemap())
        {
            if(!isInPlace)
                deleteResource(result);
            g_globalManager.getLogging()->logError("Error loading 2d texture from file '%s':\n The given file is a cubemap but is expected to be a 2d texture", m_resourceId.c_str());
            return pInPlace;
        }
        ...
        default:
            {
                if(!isInPlace)
                    deleteResource(result);
                g_globalManager.getLogging()->logError("Error loading 2d texture from file '%s':\n The given textue format is not supported", m_resourceId.c_str());
                return pInPlace;
            }
        }
...
void gep::Texture2DFileLoader::postLoad(ResourcePtr<IResource> pResource)
{
    ...
    {
        m_isRegistered = true;
        g_globalManager.getResourceManager()->registerLoaderForReload(m_resourceId, this, pResource);
    }
}
