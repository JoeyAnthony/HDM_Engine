...
#define WINDOWS_10_NO_DEBUG_DEVICE	// DirectX debugging support is now a feature on demand in Windows 10
...
void gep::Renderer::initialize()
{
    ...
    // Create the dummy 2d texture
    {
        ...
        g_globalManager.getResourceManager()->registerResourceType(m_pDummyTexture);
    }
    ...
    // Creates the dummy shader
    {
        ...
        g_globalManager.getResourceManager()->registerResourceType(m_pDummyShader);
    }
    ...
    m_pDefaultFont = g_globalManager.getResourceManager()->loadResource<Font>(FontFileLoader("data/base/dejavusans.ttf", 11));
    m_pFontShader = g_globalManager.getResourceManager()->loadResource<Shader>(ShaderFileLoader("data/base/font.fx"));
    ...
    // text billboard
    m_pTextBillboardShader = g_globalManager.getResourceManager()->loadResource<Shader>(ShaderFileLoader("data/base/fontBillboard.fx"));
    ...
    {
        m_pDummyModel = createModel(ModelType::Dummy);
        ...
        g_globalManager.getResourceManager()->registerResourceType(m_pDummyModel);
    }
...
#if defined(_DEBUG) && !defined(WINDOWS_10_NO_DEBUG_DEVICE)
...
gep::Model* gep::Renderer::createModel(ModelType type)
{
    return new Model(type, m_pd3dDevice, m_pDeviceContext);
}
...
{
    auto pModel = g_globalManager.getResourceManager()->loadResource<Model>(ModelFileLoader(path));
    return pModel;
}

void gep::Renderer::finalizeModel(Model* pModel) const
{
    //return;
    g_globalManager.getLogging()->logMessage("+++ Finalizing model.");
    if(pModel == m_pDummyModel)
        return;

    // Debug models only need the wireframe shader set.
    if (pModel->getType() == ModelType::Debug)
    {
        pModel->getMaterial(0).setShader(m_pWireframeShader);
        return;
    }

    static_assert((int)ModelType::EnumCount == 3,
                  "A new ModelType was added but this method was not updated.");
    GEP_ASSERT(pModel->getType() == ModelType::Regular, "Invalid model type for finalization.");

    // Handle the finalization for a regular model.
    //////////////////////////////////////////////////////////////////////////
    ...
    for(auto& matInfo : pModel->getMaterialInfo())
    {
        pModel->getMaterial(matId).setShader(shader);
        for(auto& tex : matInfo.textures)
        {
            if(tex.semantic == TextureType::DIFFUSE)
            {
                ...
                pModel->getMaterial(matId).addTexture(diffuseConstant, pTexture);
            }
        }
        matId++;
    }
...
gep::ResourcePtr<gep::IResource> gep::Renderer::createGeneratedTexture(uint32 width, uint32 height, const char* resourceId, std::function<void(ArrayPtr<uint8>)> generatorFunction)
{
    ...
    return g_globalManager.getResourceManager()->loadResource<Texture2D>(loader);
}
