...
inline void gep::ModelLoader::loadThModel(const char* pFilename, uint32 loadWhat)
{
    ...
    if (file.getFileVersion() < ModelFormatVersion::Version3)
    {
        std::ostringstream msg;
        msg << "File '" << pFilename
            << "' does have a old format, please reexport";
        throw LoadingError(msg.str());
    }
    ...
    //Read the size info
    {
        ...
        {
            std::ostringstream msg;
            msg << "Expected sizeinfo chunk, got '"
                << file.getCurrentChunkName()
                << "' chunk in file '"
                << pFilename << "'";
            throw LoadingError(msg.str());
        }
        ...
        if (loadWhat & Load::Meshes)
        {
            ...
            for (auto& mesh : m_modelData.meshes)
            {
...

                {
                    ...
                    {
                        if ((loadWhat & Load::TexCoords0) || (loadWhat & Load::TexCoords1) || (loadWhat & Load::TexCoords2) || (loadWhat & Load::TexCoords3))
                        {
                            uint8 numTexCoords;
                    ...
                    if (file.getCurrentChunkName() == "faces")
                    {
                        ...
                        else
                        {
                            ...
                            {
                                file.read(data);
                                face.indices[0] = (uint32)data;
                                file.read(data);
                                face.indices[1] = (uint32)data;
                                file.read(data);
                                face.indices[2] = (uint32)data;
                            }
                        }
                    }
                    else
